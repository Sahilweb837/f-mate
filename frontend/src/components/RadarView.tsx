  ...
  useEffect(() => {
    const fetchNearby = async () => {
      try {
        ...
        const maxRadius = 50; // Align with backend default
        const mapped = res.data.map((u: any, i: number) => {
           const distance = u.distance || (Math.random() * 45 + 5); // Fallback if no distance
           const normalizedRadius = (distance / maxRadius) * 0.45; // Scale to fit radar (0.45 max)
                       ...
                       <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                          ...
                          <div>
                             ...
                             <div style={{ color: "#2ecc71", fontSize: "0.7rem", fontWeight: 700, display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
                                <div style={{ width: 6, height: 6, borderRadius: '50%', background: '#2ecc71' }} /> {user.distance}km away
                             </div>
                          </div>
                       </div>
